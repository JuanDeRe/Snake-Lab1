  ...
  public final class SnakeApp extends JFrame {
    ...
    public SnakeApp() {
      ...
      for (int i = 0; i < N; i++) {
        ...
        if (i == 0 || i == 1){
          snakes.add(Snake.of(x, y, dir,false));
        }
        else {
          snakes.add(Snake.of(x, y, dir,true));
        }
      }
      ...
      this.statusLabel = new JLabel("Juego iniciado");
    ...
    private void togglePause() {
      if ("Pause".equals(actionButton.getText())) {
        ...
        new Thread(() -> {
          try {
            pauseController.awaitAllPaused();
            SwingUtilities.invokeLater(() ->{
              ...
              Snake worstSnake = snakes.stream()
                      .filter(s -> !s.isAlive())
                      .min(Comparator.comparingLong(Snake::timeAlive))
                      .orElse(null);

              String longestText = (longestAlive == null)
                      ? "No hay serpientes vivas"
                      : "Serpiente viva más larga: #" + snakes.indexOf(longestAlive)
                        + " longitud=" + longestAlive.snapshot().size();

              String worstText = (worstSnake == null)
                      ? "Ninguna serpiente ha muerto"
                      : "Peor serpiente: #" + snakes.indexOf(worstSnake)
                        + " tiempo viva=" + worstSnake.timeAlive();

              statusLabel.setText(longestText + " | " + worstText);
